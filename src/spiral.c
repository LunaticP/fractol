...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/10/21 04:25:26 by aviau             #+#    #+#             */
/*   Updated: 2016/10/21 07:25:07 by aviau            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (m.y < m.image_y)
	{
		...
		while (m.x < m.image_x / THREAD * (threads->thd + 1.0))
		{
			...
			if (m.i < max)
			{
				...
				int	r = lerp(colr[(int)i % 16], colr[(int)(i + 1) % 16], i - (long)i);
				int	g = lerp(colg[(int)i % 16], colg[(int)(i + 1) % 16], i - (long)i);
				int	b = lerp(colb[(int)i % 16], colb[(int)(i + 1) % 16], i - (long)i);
