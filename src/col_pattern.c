...
/*   Updated: 2016/11/03 17:27:47 by aviau            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	rainbow(float i, t_threads *t)
{
	...
	(void)t;
	c.r = (sin(i) > 0) ? sin(i) * 255 : 0;
	c.g = (-cos(i) > 0) ? -cos(i) * 255 : 0;
	c.b = (-sin(i) > 0) ? -sin(i) * 255 : 0;
	return (c);
}
...
t_color	orbit_trap2(float i, t_threads *t)
{
	...
	c.r = (1 - cos(floor(i))) * (t->color % 500);
	c.g = (1 - sin(floor(i) - 2.0 * 3.14 / 3.0)) * (t->color % 500);
	c.b = (1 - -cos(floor(i) - 4.0 * 3.14 / 3.0)) * (t->color % 500);
	return (c);
}
